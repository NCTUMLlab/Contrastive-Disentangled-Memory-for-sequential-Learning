...
class TransformerEncoder(FairseqEncoder):
    ...
    def showAttentionMap(self):

        return self.map_

    def showToken(self):
        
        return self.token_.detach().numpy()
    ...
    def forward_scriptable(
        ...
        n = 20
        n_top = 10
        x_all = torch.split(x,n, dim=0)
        mask_all = torch.split(encoder_padding_mask,n,dim=1)
        ...
        for i in range( len(x_all) ):
            ...
            if i == 0:
                ...
                for k,layer in enumerate(self.layers):
                    x_temp  = layer(x_temp , encoder_padding_mask=mask_all[i] if has_pads else None)
                    if k ==0 :
                ...
                mask_temp = torch.cat((mask_all[i-1][:,-n_top:],mask_all[i]),dim=1)

                output[0] =  torch.cat( (output[0],x_temp),dim=0 )


                for k,layer in enumerate(self.layers):

                    x_temp = output[k][-n_top-l:,:,:]

                    x_temp = layer(x＿temp , encoder_padding_mask=mask_temp if has_pads else None)[n_top:,:,:]

                    if k ==0 :
                        self.map_.append(layer.att_map_)

                    output[k+1] = torch.cat( (output[k+1],x_temp),dim=0 )
        ...
        for i in range( len(x_all) ):
            ...
            if i == 0:
                ...
                for k,layer in enumerate(self.layers):
                    ...
                    if x_all[i].shape[0] >= n_top :
                        mask_indices = torch.topk(torch.sum(layer.att_map_,2),n_top,1).indices
                        memory_indices = torch.topk(torch.sum(layer.att_map_,2),n_top,1).indices.unsqueeze(2).permute(1,0,2).repeat(1,1,x_all[i].shape[2])
			...
			compressed_memory = vae(torch.gather(x_temp,0,memory_indices))
                ...
                for k,layer in enumerate(self.layers):
                    ...
                    mask_temp = torch.cat((mask_memory[k],mask_all[i]),dim=1)

                    x_temp = torch.cat( (memory[k],x_temp),dim=0 )
                    

                    
                    x_temp = layer(x＿temp , encoder_padding_mask=mask_temp if has_pads else None)
